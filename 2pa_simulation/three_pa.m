...
%rods
sigma_rh_3 = 30e-95; %3p cross-sectional fluorophore area (m^6*s^2/photon^2) **very rough estimate
sigma_rh_2 = 0;%.001e-58;
...
%fluorophore
sigma_f_3 = 19e-95; %see above. https://www.janelia.org/lab/harris-lab/research/photophysics/two-photon-fluorescent-probes
sigma_f_2 = 0;%.001e-58;
...
d_fp_pos = 100e-6; %distance between focal plane and photoreceptor outer segments (m)
...
fp = 80e6; % pulse rate (Hz)
p0 = 75e-3; %laser power (W)
lambda = 1320e-9; %wavelength (m)
dwell = 12.8e-6; %time per pixel (s)
ppl = 64; %pixels per line (unitless)
lpf = 32; %lines per frame (unitless)
blank = .1; %temporal blanking fraction (unitless)
flyback = 1e-3; %frame flyback time (s)
scanfield = [178 178]*1e-6; %scanning area (m)
...
ne_2 = p0.^2 .* sigma_f_2 ./ (tp.*fp.^2) .* (pi .* Ana.^2 ./ (h.*c.*lambda)).^2; %emitted photons per fluorophore per pulse
ne_3 = sigma_f_3 ./ tp.^2 .* (p0 ./ fp .* pi .* Ana.^2 ./ (h.*c.*lambda)).^3;
ne_t = (ne_2 + ne_3) .* fp .* conc_f .* ps .* L; %emitted photons per second
...
equiv_2p = sigma_rh_2 .* (p0 ./ Apos).^2 ./ (tp .* fp) .* (lambda ./ (h.*c)) .^2 .* rhr; %R*/rod/s
...
equiv_3p = sigma_rh_3 .* (p0 ./ Apos).^3 ./ (tp .* fp) .* (lambda ./ (h.*c)) .^3 .* rhr; %R*/rod/s
...
spread = sigma_rh_3 .* intensity.^2 ./ (tp .* fp) .* (lambda ./ (h.*c)) .^2 .* rhr;
